...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        try:
            ...
            # 📌【核心重大更新】：通过网页里的播放列表标签块进行动态切割
            # 常见标签如 <div class="playlist..."> 或 <ul class="stui-content__playlist...">
            list_blocks = re.findall(r'(<(?:ul|div)[^>]*?playlist[^>]*?>.*?<\/(?:ul|div)>)', html_text, re.S)
            
            # 如果没抓到明显的 playlist 样式块，则尝试按大类 tab 切分播放源标题
            if not list_blocks:
                list_blocks = re.findall(r'(<ul[^>]*?active[^>]*?>.*?</ul>)', html_text, re.S)

            if list_blocks:
                # 提取可能存在的播放源名字（比如从 tab 菜单里提取）
            ...
            # 📌【终极兜底保障】：万一上述结构全失效了，按照原逻辑退回并拆分为单条线路
            if not play_urls:
